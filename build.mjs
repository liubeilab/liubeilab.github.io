  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="tools">
        ...
        <article class="tool">
          <img class="toolfig" src="/assets/img/tool-microscopy.jpg" alt="A microscope light path: laser excitation reflects off a dichroic mirror up through the objective into a dish of living cells, the emission returns to the camera, and the recorded images are segmented and tracked over time" loading="lazy" />
          <div class="tool__body">
            <p class="tool__tag">Observe</p>
            <h3>Microscopy</h3>
            <p>Imaging systems built for live, quantitative measurement — single-molecule, light-sheet, fluorescence-lifetime and structured-illumination approaches, often built in-house.</p>
          </div>
        </article>
...
hydrate(document.getElementById('pubs'), getPublications, pubs => pubs.map(p => \`
  <article class="pub"\${p.featured ? ' data-featured' : ''}>
    <div class="pub__year">\${esc(p.year)}<span class="pub__n">\${esc(p.idx)}</span></div>
    <div>
